...
function Pager_Nav($data_arr,$numPage,$name,$url=''){
    if($url==''){
    ...
    if($pages >1){
        ...
        echo '<label>​<center>'.$prev.' ទំព័រទី​ '. $current .$next.'</center></label>';
        }elseif ($current == $pages){
            echo '<label>​<center>'.$prev. ' ទំព័រទី​ ' . $current . '</center></label>';
            
        }else if ($current == 1){
            echo '<label>​<center> ទំព័រទី​ ' . $current .$next. '</center></label>';
        }else{
            echo '<h3><center>Pages not found</center></h3>';
        }
    }
    
    
}
function pager_ajax_nav($data_arr,$numPage){
$count = count($data_arr);
